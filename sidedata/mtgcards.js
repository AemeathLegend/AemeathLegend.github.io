...
document.addEventListener("keydown", async function (event) {
  if (event.key === "c") {
    try {
      ...
      const resultingText = await decryptAccessCode(
        CreatorModeEncyrpted,
        password,
      );
...
function addcard(parentelementtemp) {
  try {
    ...
    document.getElementById(secondparent.id).children[2].innerHTML = String(
      parseInt(document.getElementById(secondparent.id).children[2].innerHTML) +
        1,
    );
...
function removecard(parentelementtemp) {
  try {
    ...
    if (
      ...
      document.getElementById(secondparent.id).children[2].innerHTML = String(
        parseInt(
          document.getElementById(secondparent.id).children[2].innerHTML,
        ) - 1,
      );
  ...
  try {
    ...
    rowcoll.forEach((row) => {
      ...
      jsonStr.push({
        ...
        bildlink:
          tabledatainhalt[6].children[0].children[0].children[0].getAttribute(
            "src",
          ),
      });
    });
...
function getcollectionprogress() {
  try {
    ...
    datagettemp.forEach((row) => {
      ...
      yourtotalcollectedcards = yourtotalcollectedcards + amount;
    });

    if (yourcollectedcards != 0 && cardsmax > 0) {
      let percentage = (yourcollectedcards / cardsmax) * 100;

      document.getElementById("ProgressCollection").innerText =
        i18next.t("progress") + ": " + percentage.toFixed(2) + "%";
    } else {
      document.getElementById("ProgressCollection").innerText =
        i18next.t("progress") + ": 0%";
    }

    document.getElementById("totalcardsinthisset").innerText =
      i18next.t("total_cards") + ": " + yourtotalcollectedcards;
  } catch {
    window.alert(
...
function saveasfile() {
  try {
    ...
    const filename = currentSet.substring(
      currentSet.lastIndexOf("/") + 1,
...
async function getasfile() {
  try {
    ...
    const kartentabelle = document
      .getElementById("mtgcardlist")
      .querySelector("tbody");
    ...
    daten.forEach((card) => {
      ...
      let cardid = card.nummer;

      let cardan = card.anzahl;

      numinfile.forEach((idandan) => {
        if (String(idandan.nummer) === String(card.nummer)) {
          isinlist = true;
      ...
      tabellezeile.innerHTML = `
                ...
                <td id="mtgtablecell">
                    <button onclick="addcard(parentElement)">+</button>
                </td>
                <td id="mtgtablecell">
                    <button onclick="removecard(parentElement)">-</button>
                </td>
                <td id="mtgtablecell">
                    <div class="tooltip">&#128065
                        <span class="tooltiptext">
                            <img src="${card.bildlink}" alt="failed to Load Image" id="imagecard">
                        </span>
                    </div>
                </td>
            `;
...
const packImages = {
  ...
  ltrDraftBooster:
    "./sidedata/cardimages/assetssim/packs/Lord_Of_The_Rings.png",
...
async function openpackfill() {
  ...
  if (dataCheck[0] == "finDraftBooster" && dataCheck[1] == "MTG") {
    chancelist = setchancelist([10000, 3675, 700], [11, 36, 1]);
    ...
    chancelist = setchancelist(
      [1000, 833, 250, 224, 167, 55],
      [1, 6, 1, 6, 11, 36],
    );
    ...
    chancelist = setchancelist(
      [10000, 4425, 835, 285, 210, 200, 150, 50, 25],
      [1, 6, 11, 36, 1, 6, 11, 36, 1],
    );
  ...
  if (dataCheck[0] == "theLastDanceZCG" && dataCheck[1] == "ZCG") {
    ...
    chancelist = setchancelist(
      [
        1000000, 257141, 114285, 42857, 14286, 428568, 110203, 48979, 18367,
        6122,
      ],
    ...
    chancelist = setchancelist(
      [1000000, 257121, 114306, 42857, 14286, 111111, 28572, 12698, 4762, 1587],
    ...
    chancelist = setchancelist(
      [1000000, 257259, 114330, 42864, 14277, 20400, 5256, 2337, 875, 292, 146],
      [16, 17, 16, 19, 20, 26, 27, 28, 27, 28, 32],
    );
